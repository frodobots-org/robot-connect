...
const int kWristRightCameraId = 1100001;
const int kWristLeftCameraId = 1200001;
...
class TeleopMediaGst : public TeleopMediaPlugin {
 ...
 private:
  ...
  GstElement* pipeline3_;
  GstElement* appsink1_;
  GstElement* appsink2_;
  GstElement* appsink3_;
  GstElement* identity1_;
  GstElement* identity2_;
  GstElement* identity3_;
  bool is_running_;
  static GstFlowReturn OnVideoSampleMajor(GstElement* sink, gpointer user_data);
  static GstFlowReturn OnVideoSampleWristRight(GstElement* sink, gpointer user_data);
  static GstFlowReturn OnVideoSampleWristLeft(GstElement* sink, gpointer user_data);
...
GstFlowReturn TeleopMediaGst::OnVideoSampleWristRight(GstElement* sink, gpointer user_data) {
  TeleopMediaGst* self = static_cast<TeleopMediaGst*>(user_data);
  GstSample* sample = NULL;
  GstBuffer* buffer;
  GstMapInfo info;
...
GstFlowReturn TeleopMediaGst::OnVideoSampleWristLeft(GstElement* sink, gpointer user_data) {
  TeleopMediaGst* self = static_cast<TeleopMediaGst*>(user_data);
  GstSample* sample = NULL;
  GstBuffer* buffer;
  GstMapInfo info;
  ...
  if (sample) {
    ...
    gst_buffer_map(buffer, &info, GST_MAP_READ);
    //printf("send minor video data, size: %d\n", info.size - 6);
    self->SendVideoData(info.data + 6, info.size - 6, 2);
    gst_buffer_unmap(buffer, &info);
    gst_sample_unref(sample);
    return GST_FLOW_OK;
  }
...
void TeleopMediaGst::OnEvent(const char* event_name, const void* data) {
  if (strncmp(event_name, "video.user_joined", strlen("video.user_joined")) == 0) {
    ...
    if (user_id != kFrontCameraId && user_id != kWristRightCameraId && user_id != kWristLeftCameraId) {
  ...
  if (user_ids_.empty()) {
    ...
    gst_element_set_state(pipeline3_, GST_STATE_NULL);
  } else {
    ...
    gst_element_set_state(pipeline3_, GST_STATE_PLAYING);
  }
}
...
void TeleopMediaGst::Invoke() {
  gst_init(NULL, NULL);
  is_running_ = true;
  std::string encode_pipeine1 = " ! queue ! jpegdec ! videoconvert ! identity name=id ! queue ! x264enc tune=zerolatency speed-preset=ultrafast key-int-max=30 bitrate=512 bframes=0 ! appsink name=sink sync=false";
  
  std::string encode_pipeine2 = " ! queue ! jpegdec ! videoconvert ! identity name=id ! queue ! x264enc tune=zerolatency speed-preset=ultrafast key-int-max=30 bitrate=512 bframes=0 ! appsink name=sink sync=false";

  std::string encode_pipeine3 = " ! queue ! jpegdec ! videoconvert ! identity name=id ! x264enc tune=zerolatency speed-preset=ultrafast key-int-max=30 bitrate=1024 bframes=0 ! appsink name=sink sync=false";

  if (config_.count("camera1") && !config_["camera1"].empty()) {
    std::string pipeline1 = config_["camera1"] + encode_pipeine1;
  ...
  if (config_.count("camera2") && !config_["camera2"].empty()) {
    std::string pipeline2 = config_["camera2"] + encode_pipeine2;
    std::cout << pipeline2 << std::endl;
  ...
  if (config_.count("camera3") && !config_["camera3"].empty()) {
    std::string pipeline3 = config_["camera3"] + encode_pipeine3;
    std::cout << pipeline3 << std::endl;
  ...
  gst_object_unref(pipeline3_);
  gst_object_unref(appsink1_);
  gst_object_unref(appsink2_);
  gst_object_unref(appsink3_);
}
